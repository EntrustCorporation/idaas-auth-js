...
describe("parseStepUpChallenge", () => {
  it("parses acr_values from a step-up challenge header", () => {
    ...
    expect(result.acrValues).toBe("myACR");
  ...
  it("parses multiple space-separated acr_values", () => {
    ...
    expect(result.acrValues).toBe("urn:acr:bronze urn:acr:silver");
  });
  ...
  it("parses space-delimited auth-params without commas", () => {
    const header =
      'Bearer scope="protected:write" error="insufficient_scope" error_description="expected scopes within the token"';
    const result = parseStepUpChallenge(header);

    expect(result).toEqual({ scope: "protected:write" });
  });
