...
You can either pass an access token explicitly or let the SDK look one up:

- Pass `accessToken` when your application already selected the DPoP-bound token to use.
- Omit `accessToken` and pass `tokenOptions` when you want the SDK to call `getAccessToken(tokenOptions)` internally, find the matching stored DPoP-bound token, refresh it if needed, and then create the proof headers.
